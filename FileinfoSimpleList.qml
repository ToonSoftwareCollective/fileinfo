...
Item {
	...
	QtObject {
		...
		property FileinfoThreeStateButton prvButUp: butUp
		property FileinfoThreeStateButton prvButDown: butDown
		...
		function updateButtons() {
			...
			//had to do manual setting of visible property of buttons here because there is a problem with icon in FileinfoThreeStateButton when directly bound
		...
		function updateScrollbar() {
			...
			scrollbar.height = Math.floor(fileinfoMarkup.height / pageCount);
			...
			topMargin = (topMargin + scrollbar.height) >= fileinfoMarkup.height ? fileinfoMarkup.height - scrollbar.height - 1 : topMargin;
	...
	Rectangle {
		...
		Column {
			id: repeaterColumn
			width: fileinfoMarkup.width
			height: fileinfoMarkup.height
			anchors {
				top: fileinfoMarkup.top
				topMargin: 6
				left: fileinfoMarkup.left
				leftMargin: 6
			}
			spacing: 5
	...
	FileinfoThreeStateButton {
		id: butDown
		width: 38
		height: fileinfoMarkup.height / 2
		...
		anchors {
			bottom: fileinfoMarkup.bottom
			left: fileinfoMarkup.right
